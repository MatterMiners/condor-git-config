...
def test_checkout():
    with tempfile.TemporaryDirectory() as cache:
        subprocess.check_call(
            [*EXECUTABLE, REPO_URL, '--cache-path', cache]
        )
        subprocess.check_call(
            [*EXECUTABLE, REPO_URL, '--cache-path', cache]
        )
...
def test_includes():
    with tempfile.TemporaryDirectory() as cache:
        command = [
            *EXECUTABLE, REPO_URL, '--cache-path', cache, '--pattern', r'.*\.py', '--path-key', 'CHECKOUT_ROOT'
        ]
        ...
        root = next(
            line.partition(b"=")[-1].strip() for line in initial.splitlines() if b"CHECKOUT_ROOT" in line
        )
